...
const AdminDashboard = () => {
    ...
    const [activeTab, setActiveTab] = useState('products'); // 'products', 'orders', or 'on-demand'
    ...
    const fetchProducts = async (pageNum) => {
        try {
            ...
            const response = await fetch(`${API_ENDPOINTS.ADMIN_PRODUCTS}?${params.toString()}`, {
                headers: {
    ...
    const fetchOnDemandRequests = async () => {
        try {
            ...
            if (!response.ok) {
                console.error(`Failed to fetch on-demand requests: ${response.status} ${response.statusText}`);
                setOnDemandRequests([]);
                return;
            }
    ...
    const updateOnDemandStatus = async (requestId, status, adminNotes) => {
        try {
            const token = localStorage.getItem('token');
            const response = await fetch(`${API_ENDPOINTS.ADMIN_ON_DEMAND}/${requestId}`, {
                method: 'PATCH',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
        ...
        <div className={`min-h-screen ${isDark ? 'bg-gray-900 text-white' : 'bg-gray-50 text-gray-900'}`}>
            <div className="container mx-auto px-4 py-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    ...
                    <div className={`p-6 rounded-lg shadow-lg cursor-pointer hover:shadow-xl transition-shadow ${isDark ? 'bg-gradient-to-r from-orange-900 to-orange-800' : 'bg-gradient-to-r from-orange-500 to-orange-400'}`}
                        onClick={() => setActiveTab('on-demand')}
                    >
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-sm text-white">On-Demand Requests</p>
                                <p className="text-3xl font-bold text-white">{onDemandRequests.length}</p>
                                {onDemandRequests.filter(r => r.status === 'pending').length > 0 && (
                    ...
                    <div>
                        ...
                        <div className="flex gap-4 items-center mb-6">
                            ...
                            <div style={{ minWidth: '250px' }}>
                                <Select
                                    ...
                                    value={categoryFilter.map(cat => ({ value: cat, label: cat }))}
                                    onChange={(selected) => setCategoryFilter(selected ? selected.map(item => item.value) : [])}
                                ...
                                <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <input
                                            ...
                                            className={`w-full px-4 py-2 rounded border ${
                                                isDark ? 'bg-gray-700 border-gray-600' : 'bg-white border-gray-300'
                                            } ${!formData.in_stock ? 'opacity-50 cursor-not-allowed' : ''}`}
                                        />
                                        {!formData.in_stock && (
                                            <p className="text-sm text-red-500 mt-1">Stock is set to 0 when out of stock</p>
                                        )}
                                    </div>
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className={`font-semibold ${formData.in_stock ? 'text-green-500' : 'text-red-500'}`}>
                                    ...
                                    <div className="md:col-span-2">
                                        <button
                                            type="submit"
                                            disabled={editingProduct && originalFormData && JSON.stringify(formData) === JSON.stringify(originalFormData)}
                                            className={`px-6 py-3 rounded-lg font-semibold ${
                                                editingProduct && originalFormData && JSON.stringify(formData) === JSON.stringify(originalFormData)
                        ...
                        {!showAddForm && !hasMore && products.length > 0 && (
                            <div className="text-center py-8">
                                <p className={isDark ? 'text-gray-400' : 'text-gray-500'}>All products loaded</p>
                            </div>
                        )}
                    </div>
                )}
                            ...
                            {loading ? (
                                ...
                                <div className="overflow-x-auto">
                                    <table className="w-full">
                                        ...
                                        <tbody>
                                            {orders.map((order) => (
                                                <tr key={order.id} className={`border-t ${isDark ? 'border-gray-700' : 'border-gray-200'}`}>
                ...
                {activeTab === 'on-demand' && (
                    <div>
                        <h2 className="text-2xl font-bold mb-6">On-Demand Product Requests</h2>
                        <div className={`rounded-lg overflow-hidden shadow-lg ${isDark ? 'bg-gray-800' : 'bg-white'}`}>
                            {loading ? (
                                    ...
                                    <table className="w-full">
                                        ...
                                        <tbody>
                                            {onDemandRequests.map((request) => (
                                                <tr key={request.id} className={`border-t ${isDark ? 'border-gray-700' : 'border-gray-200'}`}>
                                                    <td className="px-4 py-3">{request.id}</td>
                                                    <td className="px-4 py-3">
                                                        <div>
                                                            <div className="font-semibold">{request.customer_name || request.user_name}</div>
                                                            <div className="text-sm text-gray-500">{request.customer_email || request.user_email}</div>
                                                        </div>
                                                    </td>
                                                    <td className="px-4 py-3">
                                                    ...
                                                    <td className="px-4 py-3">
                                                        {request.estimated_price ? `₹${request.estimated_price}` : 'Not specified'}
                                                    </td>
                                                    <td className="px-4 py-3">
                                                        <select
                                                            ...
                                                            onChange={(e) => {
                                                                ...
                                                                updateOnDemandStatus(request.id, newStatus, notes || '');
                                                            }}
...
Estimated Price: ${request.estimated_price ? `₹${request.estimated_price}` : 'Not specified'}
