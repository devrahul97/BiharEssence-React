...
const ReviewPopup = () => {
    ...
    return (
        <div className="fixed inset-0 flex items-center justify-center z-50 p-4">
            <div className={`relative max-w-md w-full rounded-lg shadow-2xl p-6 ${isDark ? 'bg-gray-800' : 'bg-white'}`}>
                {/* Close Button */}
                <button 
                    onClick={handleClose}
                    className={`absolute top-3 right-3 text-2xl font-bold hover:scale-110 transition-transform ${isDark ? 'text-gray-400 hover:text-white' : 'text-gray-500 hover:text-black'}`}
                >
                    ✕
                </button>
                ...
                <p className={`text-center text-xs mb-4 ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>
