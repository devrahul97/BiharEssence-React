...
const Products = () => {
    ...
    const bannerMessages = [
        ...
        { 
            ...
            image: "https://images.unsplash.com/photo-1416339442236-8ceb164046f8?w=1200&q=80"
        }
    ];
        ...
        if (isExpanded) {
            ...
            if (!categoryProducts[category]) {
                ...
                try {
                    ...
                    const response = await fetch(`http://localhost:5000/api/products?category=${category}&limit=1000`);
        ...
        return (
            <div className={`min-h-screen p-4 ${isDark ? 'bg-gray-900' : 'bg-gray-50'}`}>
                <div className="max-w-7xl mx-auto">
                    <div className={`h-12 rounded w-96 mb-6 animate-pulse ${isDark ? 'bg-gray-700' : 'bg-gray-200'}`}></div>
                    <div className={`p-4 rounded-lg shadow-md mb-6 ${isDark ? 'bg-gray-800' : 'bg-white'}`}>
                        <div className="flex flex-col md:flex-row gap-4">
                            <div className={`flex-1 h-10 rounded animate-pulse ${isDark ? 'bg-gray-700' : 'bg-gray-200'}`}></div>
                            <div className={`w-40 h-10 rounded animate-pulse ${isDark ? 'bg-gray-700' : 'bg-gray-200'}`}></div>
                        </div>
                    </div>
    ...
    return (
        <div className={`min-h-screen p-4 ${isDark ? 'bg-gray-900' : 'bg-gray-50'}`}>
            <div className="max-w-7xl mx-auto">
                <h1 className={`text-3xl font-bold mb-6 ${isDark ? 'text-white' : ''}`}>BiharEssence - {t.quickCommerce}</h1>
                
                {/* Rotating Banner */}
                <div className="mb-6 overflow-hidden rounded-lg sm:rounded-xl shadow-2xl">
                    <div 
                ...
                <div className={`p-4 rounded-lg shadow-md mb-6 ${isDark ? 'bg-gray-800' : 'bg-white'}`}>
                    <div className="flex flex-col md:flex-row gap-4">
                        <input
                            ...
                            className={`flex-1 border rounded-md px-4 py-2 focus:ring-2 focus:ring-green-500 focus:border-transparent ${isDark ? 'bg-gray-700 border-gray-600 text-white placeholder-gray-400' : 'border-gray-300'}`}
                        />
                        <div style={{ minWidth: '250px' }}>
                            <Select
                                ...
                                onChange={(selected) => setSelectedCategory(selected ? selected.map(item => item.value) : [])}
                                placeholder={`${t.filterByCategory || 'Filter by categories'}...`}
                                styles={{
                                    control: (base) => ({
                                        ...base,
                ...
                {viewMode === 'category' ? (
                    <div className="space-y-8">
                        {Object.keys(groupedProducts).map(category => {
                            ...
                            return (
                                <div key={category} className={`rounded-lg p-6 shadow-lg ${isDark ? 'bg-gray-800' : 'bg-white'}`}>
                                    {/* Category Header */}
                                    <div className="flex justify-between items-center mb-4">
                                        ...
                                        {categoryProducts.length > 4 && (
                                            ...
                                            >
                                                {isExpanded ? '← Show Less' : `See All (${categoryProducts.length}) →`}
                                            </button>
                                        )}
                                    </div>
                    ...
                    <>
                        <div className="flex flex-wrap justify-center">
                            {filteredProducts.length > 0 ? (
                                ...
                                <div className="text-center py-12">
                                    <p className={`text-lg ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>{t.noProductsFound}</p>
                                </div>
                            )}
                        </div>
                        ...
                        {!hasMore && products.length > 0 && (
                            <div className="text-center py-8">
                                <p className={isDark ? 'text-gray-400' : 'text-gray-500'}>{t.reachedEnd}</p>
                            </div>
                        )}
                    </>
                )}
                ...
                {/* About Us Section */}
                <div className={`mt-12 rounded-lg p-8 shadow-md ${isDark ? 'bg-gradient-to-r from-gray-800 to-gray-700' : 'bg-gradient-to-r from-green-50 to-blue-50'}`}>
                    <div className="max-w-3xl mx-auto text-center">
                        <h2 className={`text-3xl font-bold mb-4 ${isDark ? 'text-white' : 'text-gray-800'}`}>{t.aboutUs}</h2>
                        ...
                        <div className={`mt-6 flex justify-center gap-8 text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                            <div>
